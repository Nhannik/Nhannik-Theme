...
interface SelectType extends FC<SelectPropsType> {
  ...
  // Trigger
  CustomTrigger: typeof CustomTrigger;
...
interface GroupPropsType
  extends React.ComponentPropsWithoutRef<typeof SelectPrimitives.Group> {}
...
>(
  (
    { state = "active", size = "sm", children, disabled, className, ...props },
    ref
  ) => {
    return (
      <SelectPrimitives.Trigger
        className={cn(
          "group w-[16em] flex justify-between items-center rounded-md select-none px-4 border cursor-pointer text-base outline-none transition-colors",
          size == "md"
            ? "py-2 min-h-[2.5em]"
            : size == "lg"
            ? "py-3 min-h-[3em]"
          ...
          state == "error"
            ...
            : "border-transparent",
          disabled
            ? "bg-field-disabled border-border-disabled cursor-not-allowed pointer-events-none text-text-disabled border-transparent"
            : "bg-field-Background hover:bg-field-hover text-base text-text-primary focus-visible:outline-blue-500",
          className
        )}
        ref={ref}
        disabled={disabled}
        ...
        <div className="flex items-center gap-2">
          {!disabled && state == "error" ? (
            ...
            <HiExclamationTriangle className="text-icon-warning h-5 w-5" />
          ) : null}
          <SelectPrimitives.Icon
            children={
              <HiChevronDown className="w-5 h-5 transition-transform group-data-[state=open]:rotate-180 " />
            }
          />
        </div>
      </SelectPrimitives.Trigger>
...
// Trigger type
interface CustomTriggerPropsType
  extends React.ComponentPropsWithRef<typeof SelectPrimitives.Trigger> {}

const CustomTrigger = forwardRef<
  React.ElementRef<typeof SelectPrimitives.Trigger>,
  CustomTriggerPropsType
>((props, ref) => {
  return <SelectPrimitives.Trigger asChild ref={ref} {...props} />;
});

CustomTrigger.displayName = SelectPrimitives.Trigger.displayName;
Select.CustomTrigger = CustomTrigger;
      ...
      <SelectPrimitives.Portal
        children={
          <SelectPrimitives.Content
            className={cn(
              "group",
              "relative z-50 w-[17.4em] max-h-[15.4em] sm-scrollbar overflow-y-auto rounded-md shadow-md bg-field-Background data-[state=open]:translate-x-0  data-[state=open]:translate-y-0",
              position == "popper" &&
                "data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1 transition-transform",
              "data-[side=top]:bottom-2 data-[side=left]:right-1 data-[side=bottom]:top-1 data-[side=right]:left-1",
              className
            )}
            data-size={size}
            data-side={side}
...
>(({ className, children, ...props }, ref) => {
  return (
    <SelectPrimitives.Item
      ...
      children={
        <>
          <SelectPrimitives.ItemText children={children} />{" "}
          <SelectPrimitives.ItemIndicator
            asChild
            children={<HiCheck className="w-5 h-5 text-icon-blue" />}
          />{" "}
        </>
      }
      {...props}
...
// CustomItem type
interface CustomItemPropsType
  extends React.ComponentPropsWithoutRef<typeof SelectPrimitives.Item> {}

Select.CustomItem = forwardRef<
  React.ElementRef<typeof SelectPrimitives.Item>,
  CustomItemPropsType
>(({ className, ...props }, ref) => {
  return (
    <SelectPrimitives.Item
      className={cn(
        "w-full select-none outline-none cursor-pointer rounded-md py-1 px-2 mb-1",
        className
      )}
      ref={ref}
      {...props}
...
interface SeparatorPropsType
  extends React.ComponentPropsWithoutRef<typeof SelectPrimitives.Separator> {}
...
interface LabelPropsType
  extends React.ComponentPropsWithoutRef<typeof SelectPrimitives.Label> {}
...
>(({ className, ...props }, ref) => {
  return (
    <SelectPrimitives.Label
      className={cn(
        "py-1 px-4 text-sm select-none font-semibold text-text-secondary text-opacity-50",
        className
      )}
      ref={ref}
      {...props}
