...
function Section({ text, value, on, width, selected, disabled, size = "sm", icon, IconOnly }: sectionType) {
	return (
		<>
			<div
			...
			>
				...
				{icon ? <div className="hidden sm:block ">{createElement(icon)}</div> : null}
			</div>
		</>
	);
}
...
export default function ContentSwitcher({ size, list, value, onChange, width = "", disabled, IconOnly }: Props) {
	...
	return (
		<>
			<div
				className={twMerge(
					"max-w-full h-max rounded-[4px] border-[1px] border-solid border-Background-brand-dark flex items-center",
					disabled ? "bg-button-disabled" : "bg-notification-information bg-opacity-10",
				)}
			>
				{list &&
					list.map((e, i, a) => (
