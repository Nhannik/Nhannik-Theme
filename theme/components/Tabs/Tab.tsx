  ...
  return (
    <>
      <div
        ...
        className={twMerge(
          "w-max  px-4 flex transition-all justify-center items-center text-base select-none gap-2 ",
...
export default function Tabs({
  ...
  const tabsList2: List[] = useMemo(
    () => (overflow ? tabsList.slice(po, po + overflow) : []),
    [po]
  );
    ...
    <div className={twMerge("flex ", vertical ? "flex-col" : "items-center")}>
      {overflow && !vertical ? (
        <>
          ...
          <button
            disabled={po + overflow != tabsList.length ? disabled : true}
            onClick={add}
            className={twMerge(
