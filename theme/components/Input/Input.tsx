...
export default function Input({ inputClassName,className,state, text, label,disabled, size = "sm", ...props }: Props) {
    ...
    <div className="block">
        ...
        <div
            ...
            type="text" className={twMerge(
                ...
                state == 'error' || state == 'warning' && !disabled ? 'outline-none outline-0 rounded-l-md' : "rounded-md",inputClassName
            )} {...props} />
            {
...

        {
            text ?
                <div className="mt-1">
                    <span className={twMerge("text-md ",
